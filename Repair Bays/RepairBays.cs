...
public class RepairBays
{
    ...
    private static RepairBays? _instance;

    public static RepairBays Instance
    {
        get
        {
            _instance ??= new RepairBays();
            return _instance;
        }
    }
    
    public static Settings modSettings = new();

    public static void Init(string modDir, string modSettings)
    {
        Harmony.CreateAndPatchAll(Assembly.GetExecutingAssembly(), "com.battletech.repairbays");
                
        Instance._logger = Logger.GetLogger("RepairBays");
    ...
    {
        Instance._logger?.Log($"[{callerName}]{message}");
    }

    public static void LogError(string message, [CallerMemberName] string callerName = "")
    {
        Instance._logger?.LogError($"[{callerName}]{message}");
    }

    public static void LogError(string message, Exception e, [CallerMemberName] string callerName = "")
    {
        Instance._logger?.LogError($"[{callerName}]{message}", e);
    }

    public static void LogError(Exception e, [CallerMemberName] string callerName = "")
    {
        Instance._logger?.LogError($"[{callerName}]", e);
    }
}
...
{
    private static void Prefix(ref bool __runOriginal, SimGameState __instance, bool passDay)
    {
        if (!__runOriginal) {
            return;
        }
...
public static class TaskTimelineWidget_RefreshEntries
{
    public static void Postfix(TaskTimelineWidget __instance, SimGameState ___Sim)
    {
        ...
        foreach (WorkOrderEntry workOrderEntry in ___Sim.MechLabQueue)
        {
            ...
            if (!__instance.ActiveItems.TryGetValue(workOrderEntry, out var taskManagementElement))
            {
                continue;
            }
        ...
        while (indexTaskBay1 != -1)
        {
            ...
            if ((indexTaskBay3 != -1) && (workRemaining[indexTaskBay3] <= 0))
            {
                ...
                // sanity checks
                if (!__instance.ActiveItems.TryGetValue(workOrderEntry, out TaskManagementElement taskManagementElement))
                {
                    continue;
                }
